...
{
    class TopologicalSort // the whole solution is based on STATIC method 'Toposort' as such was the provided Java template. This implies the use of STATIC FIELDS
    {
        static List<int>[] adjList; // global adjacency list
        static bool[] nodes; // global array to store the nodes
        static Stack<int> order = new Stack<int>(); // this is for storing the TOPOLOGICAL SORT
        ...
        static void DFS(int v) // helper function that will look around the given VERTEX and find all other connected VERTICES (direct and indirect)
        {
            nodes[v] = true;
