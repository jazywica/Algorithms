...
namespace _09_CurrencyExchange // Simple example with Bellman-Ford algorithm and Negative Cycles on a directed graph G(V, E) and negative weights, based on the _09_CurrencyExchange.JPG
{
    ...
    // Negative cycles have a (logaritmic) negative sum, which is easy to dect, as it endlessly decreases the numbers within it. 
    ...
    class Bellman_Ford
    {
        ...
        Stack<long> NegativeCycle(int s, int t) // based on iterative correction of distances from origin to detect if there are negative cycles in the graph
        {
            nodes[s].dist = 0; // the only non-max value to start with is going to be the start node, so when we start scanning edges we will start updating distances from the start implicitly
            int n = adj.Length;
            Stack<long> result = new Stack<long>();
            ...
            return new Stack<long>(); // if no cycles have been found we just return an empty list
        }

        Stack<long> ReconstructCycle(int s) // helper function that gathers the cycle and returns it
        {
            Stack<long> result = new Stack<long>();
            int u = s; // this is to store the initial value that should end the cycle search
            do
            {
                result.Push(u);
                u = nodes[u].path;
        ...
        void DisplayValues() // testing function that prints node properties
        {
        ...
        Stack<long> DetectInfiniteArbitrage(int s, int t) // based on iterative correction of distances from origin to detect if there are negative cycles in the graph
        {
            nodes[s].dist = 0; // the only non-max value to start with is going to be the start node, so when we start scanning edges we will start updating distances from the start implicitly
            int n = adj.Length;
            Stack<long> result = new Stack<long>();
            ...
            if (cycle.Count == 0)
            {
                Console.WriteLine("There are no cycles in the graph");
                return result;
            }


            // 2. BFS for checking if the end currency is reachable from the cycle
            q = new Queue<long>(A_nodes.ToArray()); // here we convert the SET into a QUEUE for BFS
            for (int i = 0; i < nodes.Length; i++)
                nodes[i].dist = int.MaxValue;
            nodes[q.Peek()].dist = 0; // zero distance to the starter node (first in the queue) itself
            ...
            while (q.Count > 0) // Breadth-First Search for looking for the shortest path in a graph by storing the nodes that the current node was discovered from
            {
                ...
                {
                    if (nodes[v].dist == int.MaxValue)
                    {
                        q.Enqueue(v);
                        nodes[v].dist = nodes[u].dist + 1;
                        nodes[v].path_bfs = (int)u; // here we store the shortest path, which in the case of Arbitrage is better
                    }
                }
            }
            ...
            if (result.Count == 0)
            {
                Console.WriteLine("The end node is not reachable from the cycle");
                return result;
            }

            // 3. Checking path from the start node, it is enough to prove that we have reached the cycle
            q = new Queue<long>(new long[] { s });
            for (int i = 0; i < nodes.Length; i++)
                nodes[i].dist = int.MaxValue;
            nodes[s].dist = 0; // zero distance to the starter node itself
            ...
            while (q.Count > 0) // Breadth-First Search for looking for the shortest path in a graph by storing the nodes that the current node was discovered from
            {
                ...
                foreach (int v in adj[u])
                {
                    ...
                    if (cycle.Contains(v)) // as soon as we discover there is a connection we push the values onto the stack and return it
                        return result;
                }
            }

            Console.WriteLine("Values after second BFS:");
            DisplayValues_BFS();
        ...
        Stack<long> ReconstructArbitrage(int u)
        {
            ...
            while (u != -1)
            {
                if (cycle.Contains(u))
                {
                    result.Push(u); // the loop above stops when reaching the start node, so if we want, we can include it here
                    return result;
                }
                result.Push(u);
                u = nodes[u].path;
        ...
        void DisplayValues_BFS() // testing function that prints shortest paths and compares fastest and shortest paths
        {
            for (int i = 0; i < nodes.Length; i++)
        ...
        static void Main(string[] args)
        {
            ...
            Stack<long> routeCyclic = graphCyclic.NegativeCycle(0, 6);
            ...
            Console.WriteLine("\nInfiniteArbitrage:");
            Bellman_Ford graphArbitrage = new Bellman_Ford(adjCyclic, adjCyclic_cost);
            Stack<long> routeArbitrage = graphArbitrage.DetectInfiniteArbitrage(0, 6);
            foreach (int x in routeArbitrage)
                Console.Write(x + " ");
            Console.WriteLine();
