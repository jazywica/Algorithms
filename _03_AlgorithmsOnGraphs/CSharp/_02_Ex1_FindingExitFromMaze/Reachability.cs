...
namespace _02_Ex1_FindingExitFromMaze // DECOMPOSITION OF GRAPHS 1: EXCERCISE 1 - Given an undirected graph and two distinct vertices 𝑢 and 𝑣, check if there is a path between 𝑢 and 𝑣
{
    class Reachability // the whole solution is based on STATIC method 'Reach' as such was the provided Java template. This implies the use of STATIC FIELDS
    {
        ...
        static int Explore(List<int>[] adj, bool[] nodes, int x, int y)
        {
            ...
            foreach (int w in adj[x])
                if (!nodes[w] && !isFound) // if we haven't explored the vertex yet (false) AND THE CONNECTION IS NOT DISCOVERED YET, then we go further
                {
                    ...
                    Explore(adj, nodes, w, y); // here we call 'explore' recursively with a new neighbor and the old 'y' value
                }
        ...
        static void Main(string[] args)
        {
            ...
            List<int>[] adj = new List<int>[n]; // ADJACENCY LIST for the GRAPH, it has the same amount of elements as there are NODES
            for (int i = 0; i < n; i++)
                adj[i] = new List<int>();
            ...
            string[] xy = Console.ReadLine().Split(); // the last line is for storing the 'u' and 'v' VERTICES which represent entrance and exit from the maze
            x = int.Parse(xy[0]) - 1;
            y = int.Parse(xy[1]) - 1;

            Console.WriteLine(Reach(adj, x, y)); // Good job! (Max time used: 0.06/1.50, max memory used: 8585216/536870912.)
