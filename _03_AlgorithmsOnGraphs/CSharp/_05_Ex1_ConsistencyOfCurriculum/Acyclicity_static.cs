...
{
    class Acyclicity // the whole solution is based on STATIC method 'Acyclic' as such was the provided Java template. This implies the use of STATIC FIELDS
    {
        static List<int>[] adjList; // global adjacency list
        static bool[] nodes; // global array to store the nodes
        static int cycleValue; // field for storing a potential cycle inside a group
        static bool cyclic; // flags if we have found a a cycle in a graph
        ...
        static void Explore(int v) // helper function that will look around the given VERTEX and find all other connected VERTICES (direct and indirect)
        {
            if (adjList[v].Contains(cycleValue)) { cyclic = true; return; } // checks if the initial node value reappears and returns immediately
